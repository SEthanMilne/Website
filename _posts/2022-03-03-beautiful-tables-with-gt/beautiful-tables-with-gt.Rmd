...
The data details donations from two related campaigns: Freedom Convoy 2022, and Adopt-a-Trucker.
```{r}
GSG_data <- here::here("_posts",                "2022-03-03-beautiful-tables-with-gt",
                       "cleanGSG.csv") |>
    read_csv()
```
# Table Creation
...
```{r}
oldest_donation <- GSG_data$relative_date |>
    min()

youngest_donation <- GSG_data$relative_date |>
    max()
...
time_interval <- paste(
    ...
    day(oldest_donation),
    "th ",
    hour(oldest_donation),
    ":",
    minute(oldest_donation),
    " — ",
    month(youngest_donation, label = TRUE, abbr = FALSE),
    " ",
    day(youngest_donation),
    "th ",
    hour(youngest_donation),
    ":",
    minute(youngest_donation),
    sep = ""
)
